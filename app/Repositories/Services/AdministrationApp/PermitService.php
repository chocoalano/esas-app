...
class PermitService implements PermitInterface
{
    ...
    public function approved(int $permitId, int $authId, string $approve, string $notes): bool
    {
        ...
        foreach ($find->approvals->toArray() as $k) {
            broadcast(new MessageSent($message, $find->user_id, $k['user_id'], $url));
        }
    ...
    public function paginate(int $page, int $limit, string $search = null, int $type)
    {
        ...
        // Filter departemen jika bukan admin
        if (!$isAdmin) {
            $userDepartementId = optional($user->employee)->departement_id; // Hindari error jika employee null
            if ($userDepartementId) {
                $query->whereHas('user.employee', function ($q) use ($userDepartementId) {
                    $q->where('departement_id', $userDepartementId);
                });
            }
        }
